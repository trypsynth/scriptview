...
A compiled script starts with the 16 bytes `FasdUAS 1.101.10`. It ends with a trailer that starts with `ascr` and ends with the bytes `FA DE DE AD`. The 16-bit number six bytes before the end gives the size of the trailer. Everything between the magic and the trailer is the script.
...
- `2e`: an event (a command), with eight bytes: the suite code and the event code, such as `sysodlog` for `display dialog`.
